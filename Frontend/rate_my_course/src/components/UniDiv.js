...
export default function Uni_div({ data }) {
  ...
  if (data) {
    return (
      <Link to={`/UniversityPage/${encodeURIComponent(data.name)}`} className="w-3/4 md:w-4/12 h-24 mt-6">
        <div className='border-2 border-red-600 container shadow-lg py-2 flex items-center justify-between hover:scale-125 transition ease-in-out delay-0'>
